...
NOTE: You can also fork the project from Github and open it in your IDE or another editor.
...
Read the
https://docs.spring.io/spring-boot/reference/data/nosql.html#data.nosql.mongodb[reference docs]
...
https://www.mongodb.com/docs/mongodb-shell/[MongoDB Shell quick reference] for
...
NOTE: Notice how the previous `POST` operation includes a `Location` response header.
This contains the URI of the newly created resource. Spring Data REST also has two methods
...
You can directly query for the individual record, adapting the identifier to the one that
was created for you:
